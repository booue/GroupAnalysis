...
def read_processed_UESTCdata():
    '''
    read_processed_UESTCdata->学校层面学科排名
    评价指标:一列：平均综合得分  二列：团队数量  三列：总金额
    :return:dataArray->数据读取返回列表 
    '''
    data=pd.read_excel(r"学科分析（UESTC）.xls")
    composite_score=data["平均综合得分"]
    project_num=data["团队数目"]
    project_funding=data["总金额"]
    data_length=len(data["学科"])
    data_array=matlib.zeros((data_length,3))

    data_array[:,[0]]=np.array([list(composite_score)]).T
    data_array[:,[1]]=np.array([list(project_num)]).T
    data_array[:,[2]]=np.array([list(project_funding)]).T
    return data_array
...
# 学科排名
dataArray = read_processed_UESTCdata()
wList = [0.2,0.3,0.5]
...
for i in range(len(result)):
    newws.write(i+1,4, result[i],style) # 内容写入特定列
